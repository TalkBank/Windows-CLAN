...
void CWMPEventDispatch::OpenStateChange(long NewState) {
	IWMPMedia		*m_wmpMedia;
	double pDuration;

	if (MpegDlg != NULL && MpegDlg->mpeg != NULL) {
		if (NewState == wmposMediaOpen && MpegDlg->mpeg->isMovieDurationSet == FALSE) {
			if (MpegDlg->mpeg->m_wmpControler != NULL) {
				MpegDlg->mpeg->m_wmpPlayer->get_currentMedia(&m_wmpMedia);
				if (m_wmpMedia) {
					m_wmpMedia->get_duration(&pDuration);
				} else {
					pDuration = 0.0;
				}
				MpegDlg->mpeg->setVolume(100);
				MpegDlg->mpeg->SetRawDuration(pDuration);
				MpegDlg->mpeg->isMovieDurationSet = TRUE;
				MpegDlg->mpeg->m_wmpControler->stop();
				MpegDlg->Play();
			}
		}
	}
...
void CWMPEventDispatch::PlayStateChange(long NewState)
{
	if (MpegDlg != NULL && MpegDlg->mpeg != NULL) {
		...
		else if (NewState == wmppsPlaying) {
			if (MpegDlg->mpeg->isPlaying != 3 && MpegDlg->mpeg->isMovieDurationSet == TRUE)
				MpegDlg->mpeg->isPlaying = 1;
		} else if (NewState == wmppsStopped || NewState == wmppsPaused) {
			if (MpegDlg->mpeg->isPlaying != 3 && MpegDlg->mpeg->isMovieDurationSet == TRUE)
				MpegDlg->mpeg->isPlaying = 0;
		}
//		else if (NewState != wmppsPlaying && MpegDlg->mpeg->isPlaying == 1)
//			MpegDlg->mpeg->isPlaying = 0;
	}
	return;
}
...
void CWMPEventDispatch::PositionChange(double oldPosition, double newPosition)
{
	...
	if (MpegDlg != NULL && MpegDlg->mpeg != NULL) {
		if (MpegDlg->mpeg->isPlaying == 0 && MpegDlg->GetActiveWindow() == MpegDlg) {
			MpegDlg->SetCurPos(1);
			newPosition = newPosition * 1000.0000;
			t = (long)newPosition;
			if (newPosition > 0.0 && t < 0L)
				t = 0x7fffffff;
			MpegDlg->SetWaveTimeValue(t, 0L);
		}
	}
    return;
}
