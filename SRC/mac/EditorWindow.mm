 ...
 *
 * $Id: CLANEditorWindow.mm 260 2007-08-08 22:51:09Z richard_koch $
 *
...
@implementation CLANEditorWindow : NSWindow
/*
...
{
	if([self.myDocument fileURL] != nil) [self setTitle:@"Special Characters"];
}
*/
...
- (void)sendEvent:(NSEvent *)theEvent
{
	...
	win = [theEvent window];
	docWinController = getDocWinController(win);
	if (docWinController != nil && docWinController->isDisambiguateAgain == true) {
		docWinController->isDisambiguateAgain = false;
		[docWinController DisambiguateMorTier:docWinController];
	}
