...
/* program makes assumptions about English words in CHAT files */
extern BOOL STD_ENG_CHA;
...
void usage() {
  printf("Usage: mor [eS p q xi xl xb xc xd xa zN %s] filename(s)\n", mainflgs());
//	puts("+c : enable connl mode, output word's surface instead of stem");
...
//  puts("     MOR expects set of files Far.cut Fcr.cut (Fdr.cut)");
  ...
  puts("+q : don't treat quoted material as one object");
...
// defined in "mmaininit" and "globinit" - nomap = TRUE;
		...
		if (mor_FTime) {
			...
			isRemoveCAChar[NOTCA_OPEN_S_QUOTE] = FALSE;
			isRemoveCAChar[NOTCA_CLOSE_S_QUOTE] = FALSE;
			OverWriteFile = TRUE;
// 2012-11-1 2012-10-16 comma to %mor
...
void CloseFiles(void) {
	if (crules) { fclose(crules); crules = NULL; }
	if (drules) { fclose(drules); drules = NULL; }
	if (debug_fp != stdout) { fclose(debug_fp); debug_fp = stdout; }
}
...
static void Uniq_results(void) {
	...
	while (fgets_cr(st, SPEAKERLEN, fpin) != NULL) {
		remove_endspace(st);
    	if (uS.isUTF8(st) || uS.partcmp(st,FONTHEADER,0,0) || uS.partcmp(st,CKEYWORDHEADER,0,0))
    		continue;
...
static char preprocess_word(STRING *word) {
	...
	char *at, *dollar, *dash, res, isSkip, scat[BUFSIZ], tScat[256], isL2;
...
// *@nc	skip
	...
	*w_temp = EOS;
	*c = EOS;
	if (uS.isUpperChar(word, 0, &dFnt, MBF) && (*word != 'I' || word[1] == '_' || isalpha(word[1]))/* && !uS.isUpperChar(word, 1, &dFnt, MBF) */) { 
	...
	/* lxs lxslxs *+* */
	else if (doDefComp && uS.patmat(word,"_*+_*")) { 
		at = strchr(word,'-');
		if (at != NULL) {
			strncpy(w_temp,word,at-word);
			w_temp[at-word]=EOS;
...
static void call_analyze_lex() {
	...
	while (fgets_cr(uttline, UTTLINELEN, fpin)) {
		if (uS.isUTF8(uttline) || uS.partcmp(uttline,FONTHEADER,0,0) || uS.partcmp(uttline,CKEYWORDHEADER,0,0))
			continue;
...
*/
static char FindQuotes(int *qPos) {
	int i;

	for (i=*qPos; utterance->line[i] != EOS; i++) {
		if (uS.partwcmp(utterance->line+i, "[\"]")) {
			*qPos = i + 3;
			return(TRUE);
		}
	}
...
*/
		...
		if (chatmode == 0 || (((*utterance->speaker == '*' && !isLookAtDep) || (*utterance->speaker == '%' && isLookAtDep)) && checktier(utterance->speaker))) {
			...
			*tutt = EOS;
			...
			qPos = 0;
			if (doQuotes && !isPostCodeExclude)
				qFound = FindQuotes(&qPos);
			else
				qFound = FALSE;
			...
			while ((wi=getword(utterance->speaker, uttline, w, &wpos, wi))) {
				...
				if ((strcmp(w,".") == 0) || (strcmp(w,"?") == 0) || (strcmp(w,"!") == 0) || (*w == '+') || (*w == '-')) {
					if (doQuotes) {
						if (qFound && wi > qPos && !isPostCodeExclude) {
							qFound = FindQuotes(&qPos);
							if (!CHECK_LEXICON)
								strcat(tutt, "n|quote ");
						}
				...
				if (!isPostCodeExclude) {
					for (; last_wi <= wi; last_wi++) {
						if (doQuotes && qFound && last_wi > qPos) {
							qFound = FindQuotes(&qPos);
							if (!CHECK_LEXICON)
								strcat(tutt, "n|quote ");
						}
	...
	if (mor_ftime) {
		...
		if ((other_rules=OpenMorLib(mor_output_fname, "r", FALSE, FALSE, mFileName)) != NULL) {
			...
			while (fgets_cr(w_temp, MAXLINE, other_rules)) {
				...
				if (uS.isUTF8(w_temp) || uS.partcmp(w_temp,FONTHEADER,0,0) || uS.partcmp(w_temp,CKEYWORDHEADER,0,0))
					continue;
				if (w_temp[0] == '%' || w_temp[0] == '#')
					continue;
				if (w_temp[0] != EOS) {
					i = strlen(w_temp) - 1;
					if (w_temp[i] == ':') {
...
void hello_arrays() {
	...
	parse_stack = (PARSE_REC *) malloc((size_t) (MAX_PARSES * (size_t) sizeof(PARSE_REC)));
	if (parse_stack == NULL) {
		free(e); free(w); free(c); free(tutt); free(feat_codes);
		free(match_stack); free(word_segments); free(var_tab);
		free(result_list); out_of_mem();
	}
}
...
void bye_arrays()
{
	...
	if(parse_stack) { free(parse_stack);parse_stack = NULL;}
}
