...
static ELANCHATTIERS *RootTiers, *RootUnmatchedTiers, *RootHeaderTiers;
...
static ELANCHATTIERS *Elan_add2Headers(void) {
	ELANCHATTIERS *nt;
	
	if (RootHeaderTiers == NULL) {
		RootHeaderTiers = NEW(ELANCHATTIERS);
		nt = RootHeaderTiers;
	} else {
		for (nt=RootHeaderTiers; nt->nextTier != NULL; nt=nt->nextTier) ;
		nt->nextTier = NEW(ELANCHATTIERS);
		nt = nt->nextTier;
	}
	if (nt == NULL)
...
static void Elan_addToTiers(char *ID, const char *refID, long beg, long end, const char *refSp, char *sp, char *line, char isCreateEmpty) {
	...
	if (RootTiers == NULL) {
		if ((RootTiers=NEW(ELANCHATTIERS)) == NULL)
			out_of_mem();
		nt = RootTiers;
		nt->nextTier = NULL;
	} else {
...
static void Elan_addHeaders(void) {
	ELANCHATTIERS *unmt;
	
	for (unmt=RootHeaderTiers; unmt != NULL; unmt=unmt->nextTier) {
		Elan_addHeadersTiers(unmt->ID, unmt->refSp, unmt->sp, unmt->line);
	}
}
	...
	do {
		...
		if (!strcmp(CurrentElem->name, "TIER") && CurrentElem->data != NULL) {
			if (stackIndex < 29) {
				...
				if (parent_ref[0] == EOS && (!isMFA || tier_id[0] != '@')) {
					p = strchr(tier_id, '@');
					if (p != NULL)
...
//			p = NULL;
		...
		if (utterance->speaker[0] == '@' && beg == 0L && end == 0L) {
			...
			if (utterance->speaker[i] != ':')
				strcat(utterance->speaker, ":");
			nt = Elan_add2Headers();
			Elan_fillNT(nt, ID, 0L, 0L, refID, utterance->speaker, utterance->line, FALSE);
		} else {
			uS.remblanks(utterance->speaker);
...
//		fprintf(fpout, "%s:\t%s\n", utterance->speaker, utterance->line);
	...
	RootHeaderTiers = freeTiers(RootHeaderTiers);	
}
