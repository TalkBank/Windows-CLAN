...
static Size conv_to_msec(sndInfo *tempSound, Size num) {
	double res, num2;
...
static Size conv_from_msec(sndInfo *tempSound, Size num) {
	double res, num2;
...
static char ProcessSound(sndInfo *tempSound, SILENCE *p) {
	...
	while (cur < tempSound->SoundFileSize) {
...
#endif
		BegM = conv_to_msec(tempSound, cur);
		EndM = conv_to_msec(tempSound, cur + count);
		for (t=p; t != NULL; t=t->next) {
			if (t->end < BegM || t->beg > EndM)
				...
				silence(tempSound->SWaveBuf, conv_from_msec(tempSound, t->beg - BegM), conv_from_msec(tempSound, t->end - BegM));
			} else if (t->beg <= BegM && t->end >= BegM && t->end <= EndM) {
				silence(tempSound->SWaveBuf, 0, conv_from_msec(tempSound, t->end - BegM));
			} else if (t->beg >= BegM && t->beg <= EndM && t->end >= EndM) {
				silence(tempSound->SWaveBuf, conv_from_msec(tempSound, t->beg - BegM), count);
			}
		}
	...
	framesD1 = (double)conv_to_msec(tempSound, count) / 1000.0000;
