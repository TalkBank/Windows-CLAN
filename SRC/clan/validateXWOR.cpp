...
static char isFTime, isAnyBulletFound, *isCompFiles;
...
static void compareFiles(void) {
	...
	if (isCompFiles[0] == '-') {
		...
		if (s != NULL) {
			strcpy(ext, s);
			*s = EOS;
			fnLen = strlen(FileName2);
			compLen = strlen(isCompFiles);
			if (fnLen > compLen && uS.mStricmp(FileName2+(fnLen-compLen), isCompFiles) == 0)
				return;
			strcat(FileName2, isCompFiles);
			strcat(FileName2, ext);
		}
		strcpy(sfile.fname, FileName2);
	} else {
		strcpy(FileName2, isCompFiles);
		if (FileName2[strlen(FileName2)-1] != PATHDELIMCHR)
			strcat(FileName2, PATHDELIMSTR);
		strcat(FileName2, oldfname);
		strcpy(sfile.fname, FileName2);
	}
...
void usage() {
	...
	puts("+c4: ignore some missing bullets on %wor tier (default do not ignore bullets)");
