...
static int filter_out = ThrowAwayAll;
...
#endif
{
	...
	if ( argc<2 ) {
		...
		msg("+t  : keep all syntactic categories.\n");
		msg("+tF : use the stem tags in file F along with the other syntactic categories.\n");
		msg("-t  : use only syntactic categories defined during training with POSTTRAIN (default).\n");
