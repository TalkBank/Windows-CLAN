...
#include "registryP.hpp"

/* Creates and returns and empty directory */

Registry Registry_create( Registry_CompareFunc compare_func, Registry_HashFunc hash_func )
{
...
NORET Registry_size_hint( Registry dir, unsigned int size_hint_value )
{
  int i;
...
VOIDP Registry_get_original_key(
     Registry dir,
     CONSTVOIDP key) 
{
  RegistryRecord *p;
...
unsigned int Registry_entry_count( Registry dir )
{
...
NORET Registry_fetch_contents( Registry dir, Darray key_darray, Darray value_darray )
{
...
NORET Registry_destroy( Registry dir )
{
...
unsigned int Registry_ptrhash( CONSTVOIDP ptr, unsigned int htsize )
{
  unsigned long uns_int_ptr = (unsigned long)ptr;

  if (uns_int_ptr > 0L)
    return htsize ? (unsigned int)uns_int_ptr % htsize : 0;
  else
    return htsize ? (unsigned int)uns_int_ptr % htsize : 0;
}

unsigned int Registry_strhash(
     CONSTVOIDP strp,
     unsigned int htsize )
{
  char *cp = (char *)strp;
...
unsigned int Registry_strcasehash(
     CONSTVOIDP strp,
     unsigned int htsize )
{
  char *cp = (char *)strp;
  ...
  while (*cp != '\0') {
    if (hash_temp < 0)
      ...
      hash_temp = hash_temp << 1;
    if (isalpha(*cp) && isupper((unsigned char)*cp))
      hash_temp ^= (char)tolower((unsigned char)*cp);
    else
      hash_temp ^= *cp;
    ++cp;
  }
