     ...
     unsigned amount)
{
  ...
  if (direction == GROW_HIGH) {
    ...
    array_rep->storage = (VOIDP *)Memory_reallocate((VOIDP)array_rep->storage,
    ...
    Memory_free((VOIDP)array_rep->storage);
...
NORET Darray_hint( Darray dyn_ary, unsigned addl_hint, unsigned addh_hint )
{
  Darray_rep *temp = dlower(dyn_ary);
  int grow_amount;
...
VOIDP Darray_remh( Darray dyn_ary )
{
  Darray_rep *temp = dlower(dyn_ary);
...
VOIDP Darray_reml( Darray dyn_ary )
{
  Darray_rep *temp = dlower(dyn_ary);
  VOIDP return_value;
...
unsigned Darray_len( Darray dyn_ary )
{
...
Darray Darray_copy( Darray dyn_ary )
{
...
Darray Darray_remove( Darray dar, int index )
{
  int s, d, l;
  l = Darray_len(dar);
  if (index == (l - 1)) {
...
/*--------------------------------------------------*/
Darray Darray_duplicate( Darray a )
{
  int i, l;
  Darray ret;
...
void Darray_clear( Darray dyn_ary )
{
  Darray_rep *temp = dlower(dyn_ary);
