...
struct flupos_lines {
	...
	char *loc;
	char *tier;
	flupos_lines *nextLine;
} ;

struct flupos_where {
	unsigned int count;
	struct flupos_lines *line;
	struct flupos_where *nextMatch;
} ;

struct flupos_freqTnode {
	char *word;
	struct flupos_where *where;
	unsigned int count;
	float prolongation;	// :
	float broken_word;	// ^
	float block;		// ≠
	...
	float allDisfluency;
	struct flupos_freqTnode *left;
	struct flupos_freqTnode *right;
};
// isPOSMatch end

struct flupos_speakers {
	char isSpeakerFound;
	char isMORFound;
	char isPSDFound; // +/.
	...
	// isPOSMatch beg
	struct flupos_freqTnode *words;
	long total;				/* total number of words */
	long different;			/* number of different words */
	// isPOSMatch end

	struct flupos_speakers *next_sp;
} ;

struct flupos_tnode {
	char *word;
	int count;
	struct flupos_tnode *left;
	struct flupos_tnode *right;
};

static int  flupos_SpecWords, DBGemNum;
static char *DBGems[NUMGEMITEMS];
static char flupos_ftime, isSyllWordsList, isPOSMatch, sampleType, isMorTierFirst;
static char flupos_BBS[5], flupos_CBS[5], flupos_group, flupos_n_option, isNOptionSet, GemMode, specialOptionUsed, langType, langPrecode;
static long sampleSize;
static struct flupos_speakers *sp_head;
static struct flupos_tnode *rootWords;
static flupos_lines *rootLines;
...
// isPOSMatch beg
static flupos_lines *flucal_freeLines(flupos_lines *lines, char isJustOne) {
	flupos_lines *t;
...
static flupos_where *flucal_freeWhere(flupos_where *where) {
	flupos_where *t;
...
static void flupos_freqFreetree(struct flupos_freqTnode *p) {
	if (p != NULL) {
		flupos_freqFreetree(p->left);
		flupos_freqFreetree(p->right);
		free(p->word);
...
static struct flupos_speakers *freespeakers(struct flupos_speakers *p) {
	struct flupos_speakers *ts;
	...
	while (p) {
		...
		if (ts->ID != NULL)
			free(ts->ID);
		flupos_freqFreetree(ts->words);
		free(ts);
	}
	return(NULL);
}

static void flupos_freetree(struct flupos_tnode *p) {
	if (p != NULL) {
		flupos_freetree(p->left);
		flupos_freetree(p->right);
		free(p->word);
		free(p);
	}
}

static void flupos_error(char IsOutOfMem) {
	if (IsOutOfMem)
		fputs("ERROR: Out of memory.\n",stderr);
	sp_head = freespeakers(sp_head);
	if (SyllWordsListFP != NULL)
		fclose(SyllWordsListFP);
	flupos_freetree(rootWords);
	rootWords = NULL;
...
// isPOSMatch beg
static flupos_lines *flupos_AddNewLine(flupos_lines *lines, char *loc, char *tier) {
	flupos_lines *p;

	if ((p=NEW(struct flupos_lines)) == NULL)
		flupos_error(TRUE);
	if (loc != NULL) {
...
static char *flupos_strsave(char *s) {
	char *p;
...
static struct flupos_freqTnode *flupos_talloc(char *word, unsigned int count) {
	struct flupos_freqTnode *p;

	if ((p=NEW(struct flupos_freqTnode)) == NULL)
		flupos_error(TRUE);
	p->word = word;
	p->count = count;
	return(p);
...
static flupos_where *flupos_AddNewMatch(flupos_where *where, flupos_lines *line) {
	flupos_where *p;

	if (where == NULL) {
		if ((p=NEW(struct flupos_where)) == NULL)
			flupos_error(TRUE);
		where = p;
	} else {
		...
		if ((p->nextMatch=NEW(struct flupos_where)) == NULL)
			flupos_error(TRUE);
		p = p->nextMatch;
	}
	p->nextMatch = NULL;
...
static void flupos_addlineno(struct flupos_freqTnode *p, flupos_lines *line) {
	p->count++;
	if (line != NULL) {
		p->where = flupos_AddNewMatch(p->where, line);
	}
}

static struct flupos_freqTnode *flupos_freqTree(struct flupos_freqTnode *p, flupos_speakers *ts, char *w, flupos_lines *line) {
	int cond;
	struct flupos_freqTnode *t = p;

	if (p == NULL) {
		ts->different++;
		p = flupos_talloc(flupos_strsave(w), 0);
		p->where = NULL;
		flupos_addlineno(p, line);
		p->left = p->right = NULL;
	} else if ((cond=strcmp(w,p->word)) == 0)
		flupos_addlineno(p, line);
	else if (cond < 0)
		p->left = flupos_freqTree(p->left, ts, w, line);
	else {
		...
		if (cond == 0)
			...
			p->right = flupos_freqTree(p->right, ts, w, line); // if cond > 0
		return(t);
	}
	return(p);
}
...
static void flupos_initTSVars(struct flupos_speakers *ts) {
	ts->isMORFound = FALSE;
	ts->isPSDFound = FALSE;
...
static struct flupos_speakers *flupos_FindSpeaker(char *fname, char *sp, char *ID, char isSpeakerFound) {
	struct flupos_speakers *ts, *tsp;
	...
	if ((ts=NEW(struct flupos_speakers)) == NULL)
		flupos_error(TRUE);
...
void init(char f) {
	...
	if (f) {
		...
		rootLines = NULL;
		flupos_ftime = TRUE;
		fatal_error_function = flupos_error;
		if (defheadtier != NULL) {
...
void getflag(char *f, char *f1, int *i) {
	...
	switch(*f++) {
		...
		case 'n':
			if (*(f-2) == '+') {
				flupos_n_option = TRUE;
				strcpy(flupos_BBS, "@G:");
				strcpy(flupos_CBS, "@*&#");
				specialOptionUsed = TRUE;
			} else {
				strcpy(flupos_BBS, "@BG:");
				strcpy(flupos_CBS, "@EG:");
			}
			isNOptionSet = TRUE;
			no_arg_option(f);
			break;
...
// isPOSMatch beg
static void flupos_printlines(struct flupos_freqTnode *p) {
	flupos_where *w;
...
static void flupos_freqTreeprint(struct flupos_freqTnode *p) {
	if (p != NULL) {
		flupos_freqTreeprint(p->left);
		do {
...
static void flupos_pr_result(void) {
	char  *sFName;
	struct flupos_speakers *ts;
...
// ".xls"
			...
			if (!ts->isMORFound || ts->morSyllables == 0.0) {
				fprintf(stderr, "\n*** File \"%s\": Speaker \"%s\"\n", sFName, ts->sp);
...
static struct flupos_tnode *flupos_tree(struct flupos_tnode *p, char *word, int count) {
	int cond;
	struct flupos_tnode *t = p;

	if (p == NULL) {
		if ((p=NEW(struct flupos_tnode)) == NULL)
			flupos_error(TRUE);
		...
		p->left = flupos_tree(p->left, word, count);
	else if (cond > 0){
		for (; (cond=strcmp(word,p->word)) > 0 && p->right != NULL; p=p->right) ;
		if (cond < 0)
			p->left = flupos_tree(p->left, word, count);
		else if (cond > 0)
			p->right = flupos_tree(p->right, word, count); /* if cond > 0 */
		return(t);
	}
	return(p);
}

static char flupos_isUttDel(char *line, int pos) {
...
static char isMonoWordPreCode(char *line, int wi) {
	...
	if (langType != FRA)
		...
		return(TRUE); // 2024-05-01 because of French language
}
...
static void addToPOSList(struct flupos_speakers *ts, char *ws, char *wm) {

	rootLines = flupos_AddNewLine(rootLines, NULL, ws);
	ts->total++;
	ts->words = flupos_freqTree(ts->words, ts, wm, rootLines);
...
static int isRightText(char *gem_word) {
	...
	while ((i=getword(utterance->speaker, uttline, gem_word, NULL, i)))
		found++;
	if (GemMode == 'i')
		return((flupos_group == FALSE && found) || (flupos_SpecWords == found));
	else
		return((flupos_group == TRUE && flupos_SpecWords > found) || (found == 0));
}
...
void call()	{		/* tabulate array of word lengths */
	...
	char isRepSeg, isWWRFound, ismWWRFound, isRepeatFound;
...
*/
		...
		if (flupos_SpecWords && !strcmp(flupos_BBS, "@*&#")) {
		...
		if (*utterance->speaker == '@') {
			...
			if (uS.partcmp(utterance->speaker,"@ID:",FALSE,FALSE)) {
				if (isIDSpeakerSpecified(utterance->line, templineC, TRUE)) {
					...
					flupos_FindSpeaker(oldfname, templineC, utterance->line, FALSE);
				}
			}
			if (uS.partcmp(utterance->speaker,flupos_BBS,FALSE,FALSE)) {
				if (flupos_n_option) {
					if (isRightText(word)) {
						isOutputGem = TRUE;
					} else
			...
			} else if (found > 0 && uS.partcmp(utterance->speaker,flupos_CBS,FALSE,FALSE)) {
				if (flupos_n_option) {
				} else {
					if (isRightText(word)) {
						found--;
			...
			ts = flupos_FindSpeaker(oldfname, templineC, NULL, TRUE);
			if (isMultiLang == TRUE) {
...
*/
							...
							if (strcmp(ws, "[/]") != 0) {
								...
								if (SyllWordsListFP != NULL) {
									rootWords = flupos_tree(rootWords, ws, syllCnt);
								}
...
static void flupos_treeprint(struct flupos_tnode *p) {
	if (p != NULL) {
		flupos_treeprint(p->left);
		do {
			fprintf(SyllWordsListFP,"%s %d\n", p->word, p->count);
			if (p->right == NULL)
				break;
			if (p->right->left != NULL) {
				flupos_treeprint(p->right);
				break;
			}
			p = p->right;
		} while (1);
...
CLAN_MAIN_RETURN main(int argc, char *argv[]) {
	...
	isSyllWordsList = FALSE;
	SyllWordsListFP = NULL;
	bmain(argc,argv,flupos_pr_result);
	rootLines = flucal_freeLines(rootLines, FALSE);
	if (SyllWordsListFP != NULL) {
		flupos_treeprint(rootWords);
		fclose(SyllWordsListFP);
	}
	flupos_freetree(rootWords);
	rootWords = NULL;
