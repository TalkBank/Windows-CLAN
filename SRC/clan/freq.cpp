...
void usage() {
...
#endif
	printf("Usage: freq [cN oN %s] filename(s)\n",mainflgs());
	...
	puts("+c5: when combined with +d7 option it will search speaker tier first (default: search %%mor tier first)");
...
static freq_lines *freeLines(freq_lines *lines, char isJustOne) {
	freq_lines *t;
...
static freq_where *freeWhere(freq_where *where) {
	freq_where *t;
...
static freq_lines *AddNewLine(freq_lines *lines, char *loc, char *tier) {
	freq_lines *p;
...
// defined in "mmaininit" and "globinit" - nomap = TRUE;
		...
		if (freq_FTime) {
			...
			if (zeroMatch) {
				...
				if (wdptr == NULL && !isMorSearchListGiven() && !isLanguageSearchListGiven()) {
			...
			if (isLanguageExplicit && chatmode) {
				strcpy(ts, "%mor:");
				if (checktier(ts)) {
					linkMain2Mor = TRUE;
					FilterTier = 2;
...
static freq_where *freq_AddNewMatch(freq_where *where, freq_lines *line) {
	freq_where *p;
...
static void freq_addlineno(struct freq_tnode *p, freq_lines *line) {
	p->count++;
	if ((onlydata == 1 || onlydata == 7 || onlydata == 8) && line != NULL) {
...
static struct freq_tnode *freq_tree(struct freq_tnode *p, FREQSP *ts, char *w, freq_lines *line) {
	int cond;
...
static struct freq_tnode *freq_stree(struct freq_tnode *p, struct freq_tnode *m) {
	...
	if (p == NULL) {
		...
		if (onlydata == 1 || onlydata == 7 || onlydata == 8) {
			p->where = m->where;
		} else
			p->where = NULL;
		p->left = p->right = NULL;
...
static void freq_treeprint(struct freq_tnode *p) {
	if (p != NULL) {
		...
		do {
			if (revconc)
				RevWd(p->word);
			if (onlydata < 2 || onlydata == 7 || onlydata == 8)
...
static void freq_outputIDInfo(char *s) {
	char isBlank;
...
/*
static void freq_checktier(char *s) {
	struct tier *p;
...
static FREQSP *freq_FindSpeaker(char *sp, char *ID, char isSpeakerFound) {
	...
	if (freq_head == NULL) {
		...
		for (ts=freq_head; ts->next_sp != NULL; ts=ts->next_sp) {
...
static void defaultProcessWords(FREQSP *ts, char isLineNull) {
	...
	while ((i=getword(utterance->speaker, uttline, word, &wi, i))) {
		if (word[0] == '-' && !uS.isToneUnitMarker(word) && !exclude(word))
			continue;
		if (onlydata == 7) {
			if (utterance->speaker[0] == '%' && isMorSearchListGiven()) {
				findWholeWord(wi, templineC4);
...
//			freq_checktier(templineC2);
		...
		} else if (onlydata == 8 && uS.partcmp(utterance->speaker, "%mor:", FALSE, TRUE)) {
		...
		if (mwdptr != NULL) {
			...
			while ((i=getword(utterance->speaker, uttline, word, &wi, i))) {
				...
				if (anyMultiOrder) {
					for (tm=mwdptr; tm != NULL; tm=tm->nextword) {
						for (tm->cnt=0; tm->cnt < tm->total; tm->cnt++) {
							if (uS.patmat(word, tm->word_arr[tm->cnt])) {
						...
						if (uS.patmat(word, tm->word_arr[tm->cnt])) {
							...
							if (tm->cnt > tm->total) {
								...
								for (tm->cnt=0; tm->cnt < tm->total; tm->cnt++) {
									...
									strcat(word, tm->word_arr[tm->cnt]);
...
static void statfreq_percent_result(int percentNum, statfreq_sp_list *sp) {
	char tCombinput;
	char tisLookAtSpeakerWord;
	float diff, total;
	FNType *fn;
...
//		OverWriteFile = FALSE;
	...
	if (onlydata == 3) {
		fputs("File\tLanguage\tCorpus\tSpeaker\tAge\tSex\tGroup\tSES\tRole\tEducation\tCustom field\t", StatFpOut);
		statfreq_print_percent_row(NULL, WordsHead, percentNum, StatFpOut, NULL, NULL);
		fprintf(StatFpOut, "Types\tToken\tTTR");
		putc('\n', StatFpOut);
		while (sp != NULL) {
			fprintf(StatFpOut, "%s\t", sp->fname);
			fprintf(StatFpOut,"%s", sp->ID);
			total = 0.0;
			diff = 0.0;
			statfreq_print_percent_row(sp, WordsHead, percentNum, StatFpOut, &diff, &total);
			fprintf(StatFpOut,"%.0f\t%.0f\t", diff, total);
			if (total != 0)
				fprintf(StatFpOut,"%.3f", diff/total);
			else
				fprintf(StatFpOut,"-");
			putc('\n', StatFpOut);
			sp = sp->next_sp;
	...
	if (percent > 0) {
		...
		if (!isFoundTier) {
			...
			if (StatFp == NULL) {
				...
				statfreq_percent_result(percentNum, statfreq_head);
