...
ME_Model::Solve_BLMVM(BLMVM blmvm, BLMVMVec X){
  ...
  while (1){
  ...
  #endif
#endif
	fprintf(stderr, "%3d of %d  logl(err) = %9.6f (%6.4f)", iter, NUMITER+1, -f, _train_error);
    if (_heldout.size() > 0) {
      double hlogl = heldout_likelihood();
      fprintf(stderr, "  heldout_logl(err) = %9.6f (%6.4f)\n", hlogl, _heldout_error);
    ...
    if (sqrt(gnorm2)<gtol || iter > NUMITER || ffeval>10000) break;
    iter++;
