...
static struct utt_elems *freeUttElems(struct utt_elems *p) {
	...
	while (p) {
		ts = p;
		p = p->nextElem;
		ts->items = freeUttItems(ts->items);
		free(ts);
	}
	return(NULL);
}

static void AddItem(const char *item, char code, char phrasei) {
	struct items *p;

	if (root_elem->items == NULL) {
		...
		if ((p=NEW(struct items)) == NULL)
			flp_error(TRUE);
		p->nextItem = root_elem->items->nextItem;
		root_elem->items->nextItem = p;
	}
	p->code = code;
	p->phrasei = phrasei;
	if (item == NULL)
...
static void AddToUttElems(const char *item, char isSameThread, char code, char phrasei) {
	struct utt_elems *p;
	...
	if (root_elem == NULL) {
		...
		if (root_elem->items != NULL && isSameThread == TRUE) {
			AddItem(item, code, phrasei);
			return;
		}
		if ((p=NEW(struct utt_elems)) == NULL)
			flp_error(TRUE);
		p->nextElem = root_elem;
		root_elem = p;
	}
	p->code = code;
	p->items = NULL;
...
static void getUttSqCodes(char *line, int b,  int e, char *code) {
	code[0] = EOS;
	while (b <= e) {
		if (isspace(line[b])) {
			*code++ = ' ';
			for (b++; isspace(line[b]) && b <= e; b++) ;
		} else {
			*code++ = line[b];
			line[b] = ' ';
			b++;
...
static void addUttWord(char *line, int b,  int e, char isSameThread, char *word, char code, char phrasei) {
	int i;
	...
	word[i] = EOS;
	AddToUttElems(word, isSameThread, code, phrasei);
}

static int getCodeData(char *line, int e, char isSameThread) {
	int b, sb;
	char code, isPhrase;
	...
	getUttSqCodes(line, b, e, templineC3);
	isPhrase = FALSE;
	if (strcmp(templineC3, "[/]") == 0)
		code = ISREPEAT;
	else if (strcmp(templineC3, "[//]") == 0)
		code = ISRETRACE;
	else 
		code = ISOTHER;
//	if (line[b] == '[') {
		...
		while (e > 0) {
			if (line[e] == ']') {
				e = getCodeData(line, e, isSameThread);
			} else if (line[e] == '>') {
				isPhrase = TRUE;
				break;
			} else if (!uS.isskip(line,e,&dFnt,MBF)) {
				break;
			}
			e--;
		}
		if (isPhrase == TRUE) {
			while (e >= 0) {
				if (line[e] == ']') {
					...
					line[e] = ' ';
					AddToUttElems("<", isSameThread, code, ISBEG);
					break;
				} else if (line[e] == '>') {
					line[e] = ' ';
					AddToUttElems(">", isSameThread, code, ISEND);
				}
				while (e > 0) {
					if (line[e] == ']' || !uS.isskip(line,e,&dFnt,MBF))
						break;
					e--;
				}
				if (e < 0)
				...
				if (line[e] != '<' && line[e] != ']') {
					b = e;
					while (b >= 0) {
						if (uS.isskip(line,b,&dFnt,MBF) || line[b] == '<' || line[b] == ']') {
							b++;
							break;
						}
						b--;
					...
					if (b < 0)
						b = 0;
					addUttWord(line, b, e, isSameThread, templineC3, code, ISMID);
					if (line[b] == '<') {
						e = b;
					} else {
						e = b - 1;	
						while (e > 0) {
							if (!uS.isskip(line,e,&dFnt,MBF) || line[e] == '<' || line[e] == ']')
								break;
							e--;
						}
					}
			...
			if (b < 0)
				b = 0;
			addUttWord(line, b, e, isSameThread, templineC3, code, ISNOT);
			e = b - 1;	
		}
//	}
...
static void createUtt(char *sp, char *line) {
	...
	while (e > 0) {
		if (line[e] == ']') {
			e = getCodeData(line, e, TRUE);
		} else if (!uS.isskip(line,e,&dFnt,MBF)) {
			code = ISWORD;
			b = e;
			while (b >= 0) {
				if (uS.isskip(line,b,&dFnt,MBF) || line[b] == ']') {
					b++;
					break;
				}
				b--;
			}
			addUttWord(line, b, e, FALSE, templineC3, code, ISNOT);
			e = b - 1;
		} else
			e--;
	}
...
static void process_utt_items(struct items *item) {
	struct items *pi;

	for (pi=item->nextItem; pi != NULL; pi=pi->nextItem) {
		fprintf(stderr, "%s(%dS%d)  ", pi->item, pi->code, pi->phrasei);
	}
	fprintf(stderr, "%s(%dT%d)  ", item->item, item->code, item->phrasei);
}
...
*/
		...
		if (*utterance->speaker == '@') {
			...
			if (isPOSMatch) {
				...
				strcpy(spareTier3, utterance->line);
				createUtt(utterance->speaker, spareTier3);
				process_utt(root_elem);
...
CLAN_MAIN_RETURN main(int argc, char *argv[]) {
	...
	if (argc > 1) {
		if (!isSpGiven) {
			spArgc = argc;
			strcpy(spPref, "+t*CHI:"); // strcpy(spPref, "+t#Target_Child"); 
			argv[spArgc] = spPref;
			argc++;
		}
	}
